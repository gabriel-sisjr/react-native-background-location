...
description: Apple App Store requirements for apps using background location — Info.plist description strings, Background Modes, review guidelines, purpose strings, privacy nutrition labels, and PrivacyInfo.xcprivacy.
keywords:
  - app store
  - apple
  - ios
...
Your Xcode project must have the **Location updates** background mode enabled. This is configured in `Info.plist`:
...
Or in Xcode: **Target > Signing & Capabilities > Background Modes > Location updates**.

> See the [iOS Setup Guide](../getting-started/ios-setup.md) for step-by-step configuration.

## Guideline 5.1.1: Privacy Nutrition Labels
...
6. For each, specify collection purposes, whether linked to user identity, and whether used for tracking
7. Save and submit
...
**Solution:** Rewrite descriptions to be specific to your app's features. See the examples in the [Required Usage Description Strings](#required-usage-description-strings) section above.
...
- Use `LocationAccuracy.BALANCED_POWER_ACCURACY` or lower if you do not need precise GPS
...
- [iOS Setup Guide](../getting-started/ios-setup.md) -- Initial setup and configuration
- [iOS Background Behavior](./ios-background-behavior.md) -- How iOS handles background location
- [Google Play Compliance](./google-play-compliance.md) -- Android equivalent requirements
