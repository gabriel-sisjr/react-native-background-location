...
Replace the default import + dotted call sites with named imports. For most codebases, a single find-and-replace is enough:
...
- **Tree-shaking.** The default export bundled all tracking methods into one object, forcing every consumer to pull the entire tracking API even if they only called `isTracking()`. Named exports let modern bundlers (Metro with RAM bundles, Hermes bytecode, webpack in test environments) drop unused methods. For a typical driver app that only uses 3-4 methods, this is a measurable bundle-size win.
- **Consistency.** Geofencing (`addGeofence`, `removeGeofence`, etc.) shipped in v0.11.0 as named exports. Hooks (`useLocationPermissions`, `useBackgroundLocation`, etc.) have always been named exports. Mixing a default export with named exports forced consumers to remember two import styles and confused IDE auto-import.
...
A few gotchas we have seen real users hit. If migration feels like it should be working but isn't, check here first.
...
| -------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------- | --------- |
| `startTracking`                  | `BackgroundLocation.startTracking(...)`                     | `import { startTracking }` -> `startTracking(...)`                  | Migrated  |
| `stopTracking`                   | `BackgroundLocation.stopTracking()`                         | `import { stopTracking }` -> `stopTracking()`                       | Migrated  |
| `isTracking`                     | `BackgroundLocation.isTracking()`                           | `import { isTracking }` -> `isTracking()`                           | Migrated  |
| `getLocations`                   | `BackgroundLocation.getLocations(id)`                       | `import { getLocations }` -> `getLocations(id)`                     | Migrated  |
| `clearTrip`                      | `BackgroundLocation.clearTrip(id)`                          | `import { clearTrip }` -> `clearTrip(id)`                           | Migrated  |
| `updateNotification`             | `BackgroundLocation.updateNotification(t, x)`               | `import { updateNotification }` -> `updateNotification(t, x)`       | Migrated  |
